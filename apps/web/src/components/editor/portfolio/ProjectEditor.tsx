...
export function ProjectEditor({ data, onChange }: ProjectEditorProps) {
  ...
  const handleUpdate = (index: number, updated: PortfolioProject) => {
...
function SortableImageItem({ id, url, index, onRemove }: SortableImageItemProps) {
  ...
  return (
    ...
    >
      <img src={url} alt={`Project ${index + 1}`} className="w-24 h-24 object-cover" />
